...
extension TCPNetworkService: StreamDelegate {
    func stream(_ aStream: Stream, handle eventCode: Stream.Event) {
        streamQueue.async { [weak self] in
            ...
            case .hasBytesAvailable:
                ...
                if (aStream == self.inputStream) {
                    while ((self.inputStream?.hasBytesAvailable) != nil){
                        let len = self.inputStream?.read(&buffer, maxLength: buffer.count) ?? 0
                        if(len > 0){
            ...
            default:
                print("Connection Unknown Error with \(CommonDefine.shared.hostAddress) \(CommonDefine.shared.ipPort)")
